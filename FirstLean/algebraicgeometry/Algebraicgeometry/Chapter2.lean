...
--- A quicker proof for basisSameVariety but this needs finite indexing?
theorem basisSameVarietyNew (F G : Set (MvPolynomial σ K)) :
...
theorem inSpanAffineVarietyGeneratorsNeater {x : σ → K} (F : Set (MvPolynomial σ K)) (h : x ∈ affineVariety F) :
  ∀ p ∈ Ideal.span F, (MvPolynomial.eval x) p = 0 := by
  intro p hp
  --- This gives us that p ∈ I = <F> means that it is a linear combination of a finite number of these generators
